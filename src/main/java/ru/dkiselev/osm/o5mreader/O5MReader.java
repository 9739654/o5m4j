...
public class O5MReader {
	...
	//Statistics
	int readNodes, readWays, readRelations;
	double minLon = Double.MAX_VALUE, minLat = Double.MAX_VALUE, maxLon = Double.MIN_VALUE, maxLat = Double.MIN_VALUE;
	...
	public void read(O5MHandler handler) throws IOException {
		...
		do {
			...
			if (ds != null) {
				...
				} else if (ds instanceof Way) {
					handler.handleWay((Way) ds);
					readWays++;
					
				} else if (ds instanceof Relation) {
		...
		switch ((byte) read) {
			...
			case O5MREADER_DS_NODE: {
				...
				if (dropNodes) {
					...
					Node n = parseNode(readDSAsBytes(length, is), (int) length);
					if (n.getLon()<minLon)
						minLon = n.getLon();
	...
	private void fillTags(InputStream inputStream, NWRDataSet result, int length) throws IOException {
	...
	public Boundaries getBoundaries() {
		return new Boundaries(minLon, minLat, maxLon, maxLat);
	}
}
